...
_REDIS_CLIENTS: WeakKeyDictionary[PluginRedisFacade, Any] = WeakKeyDictionary()
...
class PluginRedisFacade:
    ...
    __slots__ = ("account_id", "plugin_key", "_prefix", "__weakref__")
    ...
    async def get(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].get(self._ns(key))
    ...
    async def set(self, key: Any, value: Any, **kwargs: Any) -> Any:
        ...
        return await _REDIS_CLIENTS[self].set(self._ns(key), value, **allowed_kw)

    async def delete(self, *keys: Any) -> Any:
        if not keys:
            return 0
        return await _REDIS_CLIENTS[self].delete(*self._ns_keys(list(keys)))

    async def exists(self, *keys: Any) -> Any:
        if not keys:
            return 0
        return await _REDIS_CLIENTS[self].exists(*self._ns_keys(list(keys)))

    async def expire(self, key: Any, seconds: int) -> Any:
        return await _REDIS_CLIENTS[self].expire(self._ns(key), int(seconds))

    async def ttl(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].ttl(self._ns(key))

    async def incr(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].incr(self._ns(key))

    async def incrby(self, key: Any, amount: int = 1) -> Any:
        return await _REDIS_CLIENTS[self].incrby(self._ns(key), int(amount))

    async def decr(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].decr(self._ns(key))

    async def decrby(self, key: Any, amount: int = 1) -> Any:
        return await _REDIS_CLIENTS[self].decrby(self._ns(key), int(amount))

    async def hget(self, key: Any, field: Any) -> Any:
        return await _REDIS_CLIENTS[self].hget(self._ns(key), field)

    async def hset(self, key: Any, field: Any = None, value: Any = None, mapping: Any = None) -> Any:
        namespaced = self._ns(key)
        if mapping is not None:
            return await _REDIS_CLIENTS[self].hset(namespaced, mapping=mapping)
        return await _REDIS_CLIENTS[self].hset(namespaced, field, value)

    async def hdel(self, key: Any, *fields: Any) -> Any:
        return await _REDIS_CLIENTS[self].hdel(self._ns(key), *fields)

    async def hgetall(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].hgetall(self._ns(key))

    async def hincrby(self, key: Any, field: Any, amount: int = 1) -> Any:
        return await _REDIS_CLIENTS[self].hincrby(self._ns(key), field, int(amount))

    async def hexists(self, key: Any, field: Any) -> Any:
        return await _REDIS_CLIENTS[self].hexists(self._ns(key), field)

    async def lpush(self, key: Any, *values: Any) -> Any:
        return await _REDIS_CLIENTS[self].lpush(self._ns(key), *values)

    async def rpush(self, key: Any, *values: Any) -> Any:
        return await _REDIS_CLIENTS[self].rpush(self._ns(key), *values)

    async def lrange(self, key: Any, start: int, end: int) -> Any:
        return await _REDIS_CLIENTS[self].lrange(self._ns(key), int(start), int(end))

    async def llen(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].llen(self._ns(key))

    async def ltrim(self, key: Any, start: int, end: int) -> Any:
        return await _REDIS_CLIENTS[self].ltrim(self._ns(key), int(start), int(end))

    async def sadd(self, key: Any, *members: Any) -> Any:
        return await _REDIS_CLIENTS[self].sadd(self._ns(key), *members)

    async def srem(self, key: Any, *members: Any) -> Any:
        return await _REDIS_CLIENTS[self].srem(self._ns(key), *members)

    async def smembers(self, key: Any) -> Any:
        return await _REDIS_CLIENTS[self].smembers(self._ns(key))

    async def sismember(self, key: Any, member: Any) -> Any:
        return await _REDIS_CLIENTS[self].sismember(self._ns(key), member)
