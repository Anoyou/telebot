...
from .rich_message import normalize_rich_message as _normalize_rich_message
...
async def send_message_draft(
    ...
    *,
    ...
    """Stream an ephemeral Bot API text draft; callers still send the final message."""

    if not isinstance(draft_id, int) or isinstance(draft_id, bool) or draft_id == 0:
        raise ValueError("draft_id 必须是非零整数")
    normalized_parse_mode = normalize_bot_parse_mode(parse_mode)
    payload: dict[str, Any] = {
        "chat_id": chat_id,
        "draft_id": draft_id,
    ...
    if normalized_parse_mode:
        payload["parse_mode"] = normalized_parse_mode
    return await call_bot_api(token, "sendMessageDraft", payload)


def normalize_rich_message(raw: Any) -> dict[str, Any]:
    """Validate Bot API Rich Message limits while preserving its character metric."""

    return _normalize_rich_message(raw, text_limit_unit="characters")
...
async def send_rich_message_draft(
    ...
    *,
    ...
    """Stream an ephemeral Bot API Rich Message draft without changing final delivery."""
    ...
    if message_thread_id is not None:
        payload["message_thread_id"] = message_thread_id
    return await call_bot_api(token, "sendRichMessageDraft", payload)
