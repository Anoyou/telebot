...
- `main` 是稳定发布线，只接收已经验证稳定的 beta；开发期间 `Beta` 必须维护带 prerelease 后缀的版本号（例如 `0.81.0-beta.1`），稳定后再合入 `main` 发布对应正式版（例如 `0.81.0`）。正式版检查点完成后，允许 `Beta` 暂时与 `main` 指向同一个稳定版提交；`Beta` 的下一次开发提交必须先切换到新的 prerelease 版本。
