...
脚本默认由 Playwright 启动 Vite。已经手动启动服务时，可运行 `VISUAL_BASE_URL=http://127.0.0.1:5173 pnpm --dir frontend exec playwright test tests/visual`。截图输出到 `docs/frontend/baseline/screenshots/`，构建分析使用 `ANALYZE=1 pnpm --dir frontend build`，一次性报告写入 `frontend/dist/build-report.html`，不提交到仓库。
