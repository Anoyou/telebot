...
## [0.99.1-beta.1] - 2026-08-24 · patch（补丁版本 Beta） · 插件消息动作反馈与发奖事件幂等修复

### 改进

- `ctx.messages.apply(...)` 返回稳定的结构化批次结果，插件可直接判断执行、失败、跳过与截断数量；旧插件忽略返回值时保持兼容。

### 修复

- 成功 payout 的可计账 ActionEvent 改由补偿单原子完成事务唯一写入，外层执行器与幂等重放不再重复提交同一 `payout_key`，消除 PostgreSQL 唯一约束错误日志。
- 消息动作部分失败时把批次结果写入插件告警并返回调用方，避免 Telegram 超时等下游错误被 facade 吞掉后让插件误报成功。

## [0.99.0] - 2026-08-23 · minor（次版本正式版） · Agent 视觉、Gateway 与平台能力正式发布

### 新增

- 上线平台能力树、账号运行模式与可持久自愈的值守流程，统一展示插件入口、平台依赖、启用状态和运行关系。
- System Agent 支持粘贴、上传、拖入图片及公开 JPG、PNG、WEBP、GIF URL，覆盖新消息、排队、运行中调整、失败重试和重新生成。
- 模拟客户端与 Gateway 支持 Chat Completions、Responses、Anthropic Messages 的原生图片输入和工具图片回放，并兼容 Codex、Claude、Grok 等客户端身份与路由语义。
- 接入 DeepSeek V4 Responses 协议，支持 `deepseek-v4-flash`、`deepseek-v4-pro`、`deepseek-v4-flash-vision-exp` 的视觉输入和原生联网搜索。
- Agent 新增贴近输入框的待处理消息队列，可在运行期间选择稍后执行或补充说明、调整方向，并持久化最近使用的模型和调用方式。

### 改进

- 重构插件中心的窄屏与 PWA 布局，优化运行模式、筛选、账号配置、插件卡片和能力树的信息密度与视觉一致性。
- 完善 Agent Markdown 长回答、原生媒体、安全图片展示及移动端排版，统一 Provider、Gateway 和模拟客户端相关术语。
- 增强插件安装历史、远端插件同步、配置备份恢复、日志查询索引与运行状态诊断。

### 修复

- 修复 PWA 流式临时消息与持久消息并存导致的重复回复，并统一普通完成、断线恢复和队列续跑的消息去重语义。
- 修复带查询参数的图片直链未进入视觉链路、Responses 工具图片块结构错误，以及 Gateway 大图片请求体边界不足的问题。
- 视觉请求日志仅记录图片数量、MIME、字节数和 SHA-256；模型在实际收到图片后仍误报无视觉能力时，会清除错误草稿并自动纠正重试一次。

### 验证

- 通过后端、前端、Gateway、API 契约、插件示例、浏览器无障碍与视觉回归测试，并完成 Web、Frontend、Updater 的 `linux/amd64`、`linux/arm64` 多架构构建验证。

## [0.99.0-beta.6] - 2026-08-23 · patch（补丁版本预发布） · 提升视觉输入稳定性

### 修复

- System Agent 明确区分模型原生图片输入与工具能力；当请求确实包含图片、模型却误报“未收到或无法查看图片”时，会清除错误的流式草稿并自动纠正重试一次，避免实验视觉模型的偶发误判直接成为最终回复。
- 视觉模型请求新增安全诊断日志，仅记录图片数量、来源类型、MIME、字节数与 SHA-256，不记录图片 URL、data URL、Base64 或图片正文；PWA 会显示明确的自动重试状态并清除首轮错误文本与思考内容。

### 测试

- 补充视觉误拒答一次性重试、无图不重试、Token 合并计量、原生视觉提示及图片日志脱敏回归测试。
...
- 修复 Agent 最终回复落库后与前端流式临时气泡短暂并存，导致 PWA 偶发显示两条相同回复的问题；最终事件现在携带持久消息 ID，并将临时气泡原位转正。
- Durable Run 恢复产生的最终回复事件同步携带消息 ID，确保断线恢复与普通流式完成使用同一套去重语义。
- 放宽 PWA 助手回复正文宽度并强化 Markdown 二、三级标题层级，改善长回答在窄屏上的阅读与排版。

## [0.99.0-beta.3] - 2026-08-17 · minor（次版本预发布） · 修复窄屏插件中心运行模式说明布局

- 修复 834px 平板视口仍显示运行模式长说明导致浏览器视觉检查失败的问题。
- 窄屏布局统一隐藏运行模式说明，保留标题、状态和展开入口。

## [0.99.0-beta.2] - 2026-08-17 · minor（次版本预发布） · Agent 任务交互与 PWA 插件中心收口

### 修复

- 修复 X/Twitter `pbs.twimg.com` 使用 `?format=jpg` 等查询参数声明图片格式时未进入 Agent 视觉输入链路的问题；公开 JPG、PNG、WEBP、GIF 直链现在会固定连接到已验证的公网 IP，安全下载后作为原生图片发送给模型。
- Agent 输入区会跨刷新和会话沿用最近一次模型与调用方式选择，避免切换会话后意外回退到旧选择。
- 任务中心只保留尚未解决的最新失败提醒，后续重试成功会自动消解旧失败；失败提醒也可手动移除，Durable Run 审计记录继续保留。
- 运行中发送的新消息会像 Codex 桌面端一样贴在输入框上方作为待处理消息，消息条内可选择“稍后执行”或“补充说明/调整方向”，并支持删除；发送区不再额外占用动作下拉按钮。
- 恢复 PWA 主内容区与设备边缘的原始外层留白，保持当前内容区内部密度和底部安全区不变。
- 优化插件中心窄屏布局：运行模式说明按需隐藏，全局筛选默认收起，账号选择与消息模板测试并排，插件计数移至标题右上角；树状插件行增加呼吸间距，编辑按钮统一为描边样式。


## [0.99.0-beta.1] - 2026-08-17 · minor（次版本预发布） · Agent 图片输入与原生媒体闭环

### 新增

- 系统助手支持上传、粘贴和拖入 JPG、PNG、WEBP、GIF 图片，也可识别消息中的公开图片 URL；图片可参与新消息、排队、运行中调整、停止替换、失败重试与重新生成。
- 模拟客户端与 Gateway 现可按 Chat Completions、Responses、Anthropic Messages 协议发送和回放原生图片块，并在 Agent 会话中展示模型生图、工具图片与安全 Markdown 图片。

### 修复

- 收敛移动端底部导航的暗色材质，移除过强白色高光与饱和玻璃效果，改善深色主题下的视觉一致性。
- 插件中心移动卡片的批量选择框改为左下角紧凑控件，减少对标题和版本信息的遮挡；桌面端保留原有位置。
...
- 移除曾作为参考实现被误提交到 `plugins/installed/lottery_plus` 的彩票插件；该目录属于本机运行时安装区，后续干净部署和生产更新会清理其匹配指纹的旧文件与遗留元数据，用户自行修改或从仓库重新安装的同名插件会保留，System Agent 文档与测试改用不落盘的 `demo_game` 示例。

### 工程

- 停止跟踪本地 `agent-plans/` 执行计划目录，并增加忽略规则回归测试，禁止再次把本地计划或 `plugins/installed/` 下的运行时插件强制提交到 Git。

## [0.97.0-beta.8] - 2026-08-16 · patch（补丁预发布） · PWA 内容密度回调

### 调整

- 将上一版 PWA 窄屏主内容区的统一边距、区块间距与卡片内边距整体放大 1.5 倍，例如外层边距由 6px 调整为 9px、常见卡片内边距由 8px 调整为 12px；保留紧凑布局，同时恢复必要的内容呼吸感，按钮高度与安全区保持不变。

## [0.97.0-beta.7] - 2026-08-16 · patch（补丁预发布） · 插件批量启停与 PWA 密度修复

### 修复

- 修复插件中心批量全局启停只识别 ZIP/旧版安装包、导致仓库与 Git 插件永久无法选中的问题；批量工具栏改为稳定换行布局，并区分加载、失败与无可选项状态。
- 将插件卡片/树状视图选择器收敛到全局 Tabs 组件，统一移动端容器背景、激活态、圆角、阴影与键盘焦点样式。
- 进一步收紧 PWA 窄屏主内容区的常用边距、间距和卡片内边距，统一按当前 spacing token 减半，为实际内容让出更多垂直空间；顶栏安全区改为不含透明度和模糊的主题实色铺底，防止 iOS PWA 状态栏出现上下分层。

### 文档与计划

- 统一插件 API 参考、Cheatsheet、开发指南与开发规则的会话契约：Event Bus 订阅入口首次建局必须先返回 `start_session`，只有当前事件已携带有效会话时才可直接 `update_session`。
- 在执行状态中补充 AI 写操作工具、AI Provider seam 与新平台枝脚手架的具体触发场景和判定标准。

## [0.97.0-beta.6] - 2026-08-15 · patch（补丁预发布） · 三通道一致性与资金运行态修复
...
- 修复插件树状视图未随所选账号切换的问题：插件启用状态与能力调用关系会即时按当前账号重新计算，同时保留平台枝的全局运行状态。
- 修复插件页窄屏下搜索、状态筛选与批量操作区域被压缩变形的问题，并为不可用的批量按钮补充明确原因。

### 改进

- 收敛树状视图文案，以“插件调用”“平台能力”等实际语义替代“取养分”等过度树状隐喻。
...
- 为插件页新增的运行模式与树状视图接口补充浏览器测试夹具，恢复无障碍测试的请求契约校验。

## [0.97.0-beta.3] - 2026-08-15 · minor（次版本预发布） · 插件工作台入口收口

### 改进

- 将运行模式值守入口合并到插件页账号插件配置卡片顶部并默认折叠，保留预检、确认、恢复与失败态。
- 在插件页提供“卡片视图（功能分类）”与“树状视图（能力分类）”切换；树状视图展示插件中文名、key 和可编辑入口。
- 移除系统设置页重复的运行模式与看树视图入口，以及插件页冗余的 AI 插件快捷入口。

## [0.97.0-beta.2] - 2026-08-15 · minor（次版本预发布） · 看树视图交互与能力语义收口

### 改进

- 看树视图重构为"侧生树"形态：树干脊柱、枝干连线与叶行导轨可视化干-枝-叶层级；叶按嫁接通道挂靠（直通/命令贴干、交互长在交互枝），AI 与资金台账作为养分枝显示"被 N 叶取养分"，额外需求以「+能力」徽标表示并支持悬停联动高亮；值守时全树降饱和并显示修枝剪徽记。

### 文档

- 修正 `requires_platform_capabilities` 指南与平台能力描述中 `ledger` 的声明条件：以发奖、结算、补偿、人工核销等资金动作为主要判据，并注明台账枝关闭或值守时资金动作 fail-closed 拒绝。

## [0.97.0-beta.1] - 2026-08-14 · minor（次版本预发布） · 平台能力组合与值守安全

### 新增

- 新增插件 `requires_platform_capabilities` 声明、按叶自动点亮平台枝与看树视图；值守或管理员强制关闭时明确拒绝启用，新装/升级强制校验声明，存量缺声明或源缺失仅提示警告。
- 新增可持久、自愈的值守运行预设：暂停插件叶投递与定时任务、冻结资金动作、停止交互 Bot 采集，并支持按进入前快照安全恢复。
- 新增 fail-closed 资金保险丝：仅在台账模块处于 ready 且无拒绝原因时允许发奖、补偿重放和人工核销；状态未知或切换中会明确拒绝新资金动作并保留待补义务。

### 文档

- 新增能力接线协议与静态 Protocol，明确注册面的命名、依赖、生命周期、所有权、generation 失效和 fail-closed 约定。

### 测试

- 覆盖纯工具叶保持台账枝关闭、游戏叶自动点亮并写审计、值守修枝剪拒绝、源缺失叶容错，以及新装/升级声明校验。
- 覆盖六个资金闸点的开关矩阵、quiescing 与冷启动断闸、deny 注册所有权清理，以及恢复后待补义务恰好结算一次。

## [0.96.0-beta.3] - 2026-08-12 · minor（次版本预发布） · 会话实时同步修复

### 修复

- 修复 AI Workspace 使用长期缓存后，移动端打开会话列表时无法在原有实时窗口内发现其他设备新建会话的问题；仅在会话列表可见时恢复短周期同步，其余状态继续保持低频刷新。

### 测试

- 通过 Playwright 覆盖移动端打开会话列表后发现远端新会话的回归场景。

## [0.96.0-beta.2] - 2026-08-12 · minor（次版本预发布） · 十项性能与管理体验优化收口

### 新增

- 插件中心新增服务端批量启停接口与安装生命周期时间线，记录安装、更新、启用、停用和卸载状态；历史仅保留版本、来源、签名等状态摘要，不复制 manifest、配置或凭据。
- 日志页新增完整筛选范围统计，支持关键词、时间、账号、插件过滤和当前加载结果 CSV 导出；Redis 仅短暂缓存脱敏后的数字聚合，并增加多账号/来源组合查询索引。
- 设置页备份恢复升级为“选择文件、事务预检、确认恢复”的一步式向导；确认时重新校验预览签名，只有成功提交后才通知 Worker 热加载。
- 生成统一的 `plugin.json` JSON Schema，并将插件 Schema、OpenAPI 和前端类型共同纳入 `make codegen` 与 CI 漂移检查。

### 改进

- AI Workspace 会话缓存改为长期有效，由 Durable Run 增量事件主动失效；任务中心在收起和空闲状态使用低频兜底同步，兼顾外部任务发现与长会话性能。
- Ledger 的 ECharts 折线图改为页面内部动态加载，日志和账本路由保持独立按需 chunk，避免图表依赖进入首屏主包。
- Worker Supervisor 按每批最多四个账号有界并发启动，展示排队/启动中状态，并隔离单账号启动失败，降低多账号同时加载 Telethon session 的资源峰值。
- System Agent 工具注册表新增按代际失效的只读列表缓存；动态插件工具参数统一执行 Draft-07 JSON Schema 校验，避免缓存用户上下文或工具调用数据。
- `/accounts` 等现有 REST 接口继续使用轻量响应模型和字段裁剪，不引入额外 GraphQL 运行时与双重契约维护成本。
- 插件安装、CLI 检查、运行时 Agent 工具和公开 Schema 统一使用同一元数据契约来源，减少模板与 MessageOps 配置语义漂移。

### 安全

- CSV 公式注入防护覆盖常见 ASCII 与 Unicode 前导空白；日志统计缓存键只保留筛选参数 SHA-256 摘要，插件安装历史不保存敏感配置。

### 测试

- 补充 Worker 有界启动、日志 Redis 命中与降级、插件生命周期、工具注册缓存、非法 Schema、恢复预检签名和 dry-run 回滚等回归测试。

## [0.96.0-beta.1] - 2026-08-12 · minor（次版本预发布） · 工作台、日志与插件体验优化

### 改进

- 插件中心新增名称/key/用途搜索、全局启停状态与当前账号异常筛选，并支持带确认、串行执行和部分失败反馈的批量全局启停。
- 日志页为消息流、Agent 运行、系统控制台和运行事件统一增加当前筛选结果 CSV 导出；补齐标准转义、对象序列化和表格公式注入防护。
- AI Workspace 根据活跃任务和队列动态调整轮询频率，空闲时停止高频拉取；会话列表改为较长缓存窗口，减少长会话场景的无效刷新。
- 插件安装与 CLI 检查统一校验 `config_schema` 和 Agent 工具参数的 JSON Schema 合法性，并补充回归测试。

## [0.95.0] - 2026-08-11 · minor（次版本） · Agent 插件数据能力与互动去重

### 新增

- System Agent 现在可读取脱敏后的插件配置 JSON，并在管理员授权确认后按配置 Schema 修改账号级或全局配置；同时新增插件私有 SQLite 数据库的结构查看与受限只读查询能力。

### 修复

- 修复“插件的配置”等自然表达只路由到安装包工具、导致 Agent 误报无法读取账号配置的问题；配置请求现在可联合读取功能矩阵、配置 Schema、生效 JSON 与交互规则。
- 修复同一交互消息已被 Interaction Bot 管道处理后仍回落到插件 legacy `on_message`、导致猜骰等奖励动作重复发送的问题；跨管道重复或去重状态无法确认时现在统一阻断旧入口。

### 安全

- 插件 SQLite 查询固定在已安装插件私有数据目录，仅允许参数化 `SELECT/WITH`，并限制管理员角色、路径、符号链接、常见密钥列、执行时间、计算量、行数和输出体积。

## [0.94.1] - 2026-08-11 · patch（补丁版本） · 互动游戏奖励修复

### 修复

- 修复插件互动规则把支付触发门槛误当成游戏奖励的问题；支持奖励字段的旧规则会迁移既有奖励配置，并确保明确奖励优先于付款金额。
...
## [0.86.1] - 2026-07-29 · patch（补丁版本） · 定时目标与工作台界面修复
...
- 定时任务通过 `@username` 连续发送时，每次重新解析包含 `access_hash` 的完整 Telegram 输入实体，不再把首次结果降级为无法稳定复用的纯整数 ID。
- AI 中心“调用成功率”改用与同组指标一致的状态卡片结构，统一标题图标、顶部状态条、内边距、数字排版和悬浮反馈。

### Tests

- 补充定时任务重复解析完整实体的回归测试，并校验移动端插件管理页不再出现“推荐插件”区域。
