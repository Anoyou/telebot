...
- 看树视图重构为"侧生树"形态：树干脊柱、枝干连线与叶行导轨可视化干-枝-叶层级；叶按嫁接通道挂靠（直通/命令贴干、交互长在交互枝），AI 与资金台账作为养分枝显示"被 N 叶取养分"，额外需求以「+能力」徽标表示并支持悬停联动高亮；值守时全树降饱和并显示修枝剪徽记；新增免登录设计预览页 `platform-tree-preview.html`。
