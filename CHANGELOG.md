...
- 阶段 C（全量已启用模型测活）：新增 `services/llm_liveness.py` 与 `POST /api/commands/llm-providers/liveness/{preview,run}` 两个端点。预览只读数据库、不调用上游、不消耗 quota，仅统计 `models[].enabled == true` 的模型并给出任务数、成本上限（任务数 × max_tokens）、缺凭据/无启用模型的跳过原因；执行采用有界公平调度（全局并发 + 单 Provider 并发 + Provider 轮询防独占），支持范围过滤（失败重测 / 指定 Provider / 指定模型）。每个测活请求都携带该 Provider 解析出的真实客户端身份 UA（与正式调用一致）；取消基于前端断开连接（轮询 `request.is_disconnected()` 置起取消令牌），在途请求自然完成、不再发起新任务。429 只把对应 Provider 并发降到 1、401 停止该 Provider 剩余任务，其它 Provider 不受影响；手工测活不修改生产 cooldown、不自动禁用模型，结果面向前端全部脱敏。前端新增全量测活弹窗（执行预览确认 + 逐项结果 + 取消）。
