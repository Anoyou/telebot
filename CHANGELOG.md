...
- **System Agent 阶段 2（核心写操作，开发中）**：新增 `system_agent_action`、统一事务执行器与重复确认保护；写工具生成待确认 Action；Web 内联 Action 卡片（确认/拒绝/重新同步）；覆盖账号暂停恢复与 Worker 重启、通用 Rule、交互规则、Scheduler、功能启停。
