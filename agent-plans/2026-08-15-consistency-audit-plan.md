# 一致性核销与文档真实性审计方案 v1.1

> 2026-08-15 定稿；同日 v1.1：三份 2026-07-09 原始设计文档确认散佚，WP-A 必读输入改为幸存摘要，第 8 项改为对当前代码重推 parity 矩阵。两个独立审计 WP：WP-A **零代码修改**，WP-B **仅文档修改**。
...
**必读输入（v1.1 修订：三份 2026-07-09 原始设计文档已散佚，不可恢复，禁止凭空重构）**：
- `agent-plans/2026-07-09-findings-survivor-summary.md`——原文结论的幸存摘要（advisor 记忆转写，0.53.x 基线），是八项清单的唯一权威输入；其中函数名/症状只作定位线索。

**第 8 项方法修订（v1.1）**：原"按 10 子项展开"依赖已散佚的漂移清单，改为**对当前代码重推 parity 矩阵**：枚举平台动作词表 × 三执行体（E1 loader 直执行 / E2 delivery / E3 worker RPC），逐格比对行为（reply_markup 处理、错误码、限流拒绝、锚点查找、去重、审计字段等维度）。以幸存的两项最重漂移为种子；若重推出的差异维度明显少于 10，视为扫描不够细而非"已收敛"。此法同时覆盖历史清单与其后新引入的漂移。
...
8. **执行体三体漂移**（E1 loader 直执行 / E2 delivery / E3 worker RPC，按上方 v1.1 修订的 parity 矩阵法展开）：历史最重两项为 E3 不读 reply_markup（userbot 按钮经 bot 路静默丢失）、错误码族与限流拒绝行为跨执行体分叉。注意 T3 资金闸已统一了 payout 拒绝面，核销时区分"T3 已覆盖"与"仍分叉"。
