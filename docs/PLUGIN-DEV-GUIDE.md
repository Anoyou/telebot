...
- `resolve_public_sender_identity()` / `resolve_public_sender_identities()` 的身份结果不做应用层缓存，每次调用都会实时读取当前群权限；Interaction Bot 兜底同样实时调用 `getChatMember`。近期发言 Redis 锚点只保存并重新校验 `message_id`，不保存姓名、username、管理员状态或标签。插件不要另行缓存这些身份字段。
