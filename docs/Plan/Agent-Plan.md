...
> 状态：阶段 1–3 与阶段 4 首批扩展均已落地，分支 `Agent-beta` 按单一 `0.64.0` 首发版本验收。
>
...
>
> 计划基线：2026-07-18，`main` 分支，TelePilot `0.63.1`。实现时以当前分支源码和项目级 `AGENTS.md` 为准；源码发生实质漂移时先更新本文档。
...
- 工具只调用允许列表内的现有业务 service，不给模型万能 SQL、Shell、文件或 HTTP 权限；插件与系统运维仅通过明确注册的危险工具开放。
...
- 首发注册 57 个工具；代理、Webhook、配置包和台账写操作等剩余长尾能力进入 backlog。
- 分四阶段实现和验收，首次公开发布统一交付为 `0.64.0`。
...
首发实际注册 57 个工具。`list/get/save` 使用筛选参数和可选 ID，避免为同一业务对象拆出大量重复工具。
...
每个阶段仍保持独立实现和验收边界；由于首次公开发布前四阶段已全部完成，本次统一作为 `0.64.0` 交付，后续扩展再按 SemVer 独立发版。
...
- 首批由用户明确指定并纳入 `0.64.0`：插件文件安装/更新/卸载、仓库维护、系统更新/重启与 AI 指令 auto 路由。
- 后续仍根据实际使用频率一次选择一个小批次，每批独立测试并按项目 SemVer 判断版本。
...
- `0.64.0` 同时发布阶段 1–3 与阶段 4 首批用户指定能力。
- 后续新增工具或工作流按实际影响使用 MINOR/PATCH，不再为历史阶段预占版本号。
...
### 26.7 剩余长尾能力进入 backlog

原因：插件安装、仓库维护和系统重启已由用户明确纳入首发；Webhook、配置导入、代理和台账写操作等剩余低频能力继续按真实需求后置。

### 26.8 Bot 与 Web 同版本首发

原因：Bot 是原始核心入口；实现阶段可以拆分，但首次公开发布时两个入口和已完成工具统一交付，减少版本维护分叉。

## 27. 实施完成定义

`0.64.0` 首发完成时，System Agent 基线能力完成：

- `/assistant` 和 `/agent` 均可用。
- 会话历史可恢复、删除。
- 固定 tools Provider 可配置并有明确错误状态。
- 57 个工具覆盖系统、账号、交互规则、Rule、Scheduler、日志、台账、Provider、指令、功能、插件包/仓库与系统运维。
