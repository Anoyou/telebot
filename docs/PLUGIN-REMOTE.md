...
| 仓库插件 | 使用者接入仓库中的普通插件，例如自动回复、图片生成、游戏或互动玩法 | 从选定仓库按需安装/更新；必须完整声明 `usage`、`event_subscriptions`、`capabilities`，刷新时保留新字段和风险提示 |
...
仓库插件不允许为了通过 lint 写空声明。至少要写清：谁能触发、订阅哪些事件、使用哪些能力、触发方式如何决定会话通道、付款/发奖是否使用 `payout`、如何在日志页按 trace 或 action 排查。
