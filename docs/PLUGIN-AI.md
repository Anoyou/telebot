...
当系统设置中 **AI 平台能力**关闭时，worker 不加载 Provider；正常热切换会把现有插件上下文的 `ctx.ai` 置为 `None`，新上下文也不会注入该 facade，因此插件应在调用前先检查 `ctx.ai`。如果切换竞态中仍有旧代码持有先前取得的 `PluginAI` facade 并继续调用，facade 会抛出结构化 `AIUnavailableError`。新插件可声明 `requires_platform_capabilities=["ai"]`，让路由层提前裁剪依赖 AI 的入口。详见 [平台能力热插拔](PLATFORM-CAPABILITIES.md)。
...
Provider 的 `execution_backend` 同样由平台配置决定，插件不能要求某次调用强制 direct 或 Gateway。选择 `codex_gateway` 的 Responses Provider 时，`complete()`、`stream_complete()` 和 `run_agent()` 仍复用同一 Client Builder：Gateway 只替换传输层，工具 handler、插件 quota、账号预算、fallback 和 usage 继续由 TelePilot 管理。同一 Provider 的 Gateway 失败不会偷跑 direct；可在尚未输出文本时按平台策略 fallback 到其它 Provider。实际 backend、Gateway 版本、request ID 和失败阶段写入「AI → 近期调用」，不加入插件可见的 routing 摘要。详见 [内置 Codex Gateway](./CODEX-GATEWAY.md)。
