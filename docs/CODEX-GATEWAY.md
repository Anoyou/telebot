...
5. Gateway 只支持文本、图片输入与工具调用的 Responses 路径。`input_image` 的 HTTP/HTTPS URL、data URL、`detail` 与工具结果内容块会原样转发；已有 `file_id` 也会保留，但 Gateway 不实现 DeepSeek Files API 上传流程。Chat Completions、Anthropic Messages、语音转写和 `/images/generations` 图片生成必须使用 direct Provider。
6. Gateway 的 Responses 请求体上限为 40 MiB，可覆盖 Agent 当前最多 4 张、每张 6 MiB 图片经 Base64 编码后的请求；上游普通响应与 SSE 累计上限保持 16 MiB，避免无界占用内存。
