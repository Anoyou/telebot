...
| AI | OpenAI Chat Completions、OpenAI Responses、Anthropic Messages 与 Ollama 兼容协议；Responses Provider 可选内置 Codex Gateway；支持模型发现、测活、路由标签、客户端身份、加密兼容请求头、调用记录、Token 与预算 |
...
- 内置 Codex Gateway 是 Web 按需拉起的独立 Go 子进程，仅通过 Unix Socket 服务选择 Gateway 的 Responses Provider；它不是额外 Compose 服务，也不接管 Agent 决策。
...
| 内置 Codex Gateway 配置、健康与排障 | [Codex Gateway](./docs/CODEX-GATEWAY.md) |
